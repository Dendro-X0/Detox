...
const REPORT_TAB_STORAGE_KEY = 'assistUnderstandReportTabId';
...
function understandPanelPath(): string {
    return 'sidepanel.html?view=understand';
}
...
async function openOrFocusReportTab(): Promise<void> {
    ...
    const url = chrome.runtime.getURL(understandPanelPath());
...
export async function openUnderstandPanel(tabId: number): Promise<void> {
    const path = understandPanelPath();
